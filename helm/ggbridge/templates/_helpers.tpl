...
{{/*
Returns the name of the secret holding the server TLS material.
This is the bring-your-own secret when tls.existingSecret is set, otherwise the
one generated by the chart. Every reference to the server certificate or to the
client CA used for mTLS must go through this helper, so that the two stay in
sync: tls-secrets.yaml does not create the generated secret when existingSecret
is set, and a hardcoded reference to it would then dangle.
{{ include "ggbridge.server.tls.secretName" $ }}
*/}}
{{- define "ggbridge.server.tls.secretName" -}}
{{- if .Values.tls.existingSecret -}}
{{ .Values.tls.existingSecret }}
{{- else -}}
{{ printf "%s-crt" (include "ggbridge.server.fullname" .) }}
{{- end -}}
{{- end -}}
...
{{- else if eq .Values.server.ingress.controller "nginx" -}}
  {{- if .Values.tls.enabled -}}
    ...
    {{- if eq (lower .Values.tls.mode) "mutual" -}}
      {{- $_ := set $annotations "nginx.ingress.kubernetes.io/auth-tls-secret" (printf "%s/%s" .Release.Namespace (include "ggbridge.server.tls.secretName" .)) -}}
